...
$internalAgent=print_internal_worker_agent($pdo);$internalOnline=$internalAgent?print_agent_online($internalAgent):false;
...
json_response(['success'=>true,'generated_at'=>(new DateTimeImmutable('now',new DateTimeZone('UTC')))->format('Y-m-d\TH:i:s.v\Z'),'overall'=>['healthy'=>$healthy,'label'=>!$setupComplete?'راه‌اندازی مسیرهای ضروری چاپ کامل نشده':($problemCount>0?'چاپ نیازمند رسیدگی است':'چاپ آماده است')],'problem_count'=>$problemCount,'internal_worker'=>['configured'=>$internalAgent!==null,'online'=>$internalOnline,'agent_id'=>(int)($internalAgent['id']??0)],'online_agents'=>$onlineAgents,'agent_count'=>count($agents),'ready_destinations'=>$readyDestinations,'destination_count'=>count($destinations),'last_submitted_at'=>print_database_time_to_utc($lastSubmitted)]);
