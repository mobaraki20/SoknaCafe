...
if (!$acceptanceStates['cafe']) {
    json_response([
        'success' => false,
        'code' => 'ordering_paused',
        'scope' => 'cafe',
        'message' => order_acceptance_message('cafe'),
    ], 423);
}

try {
    $payload = normalize_order_request_payload($data);
...
try {
    ...
    // The table lock serializes order creation and pending-session creation per table.
    ...
    if (!$table) {
        throw new GuestOrderException('invalid_qr', customer_message('invalid_qr'), 404);
    }
    ...
    if ($existing = $existingStmt->fetch()) {
        if ((int)$existing['table_id'] !== $tableId) {
            throw new GuestOrderException('invalid_order', 'شناسه این ارسال با میز دیگری ثبت شده است.', 409);
        }
        $pdo->commit();
        json_response(guest_order_response($existing, true));
    }
    ...
    if (table_sessions_enabled()) {
        if ($payload['session_token'] !== '') {
            ...
            if (!$session) {
                throw new GuestOrderException(
                    'session_inactive',
                    'نشست قبلی این میز بسته شده؛ صفحه را تازه کن و دوباره سفارش بده.',
                    409
                );
            }
        } else {
            ...
            $session = $sessionStmt->fetch() ?: null;
            if (!$session) {
                $session = create_table_session($tableId, null, null, null, 'pending');
            }
        }

        $sessionId = (int)$session['id'];
        if (settlement_session_has_active_itemized_locked($pdo, $sessionId)) {
            throw new GuestOrderException('itemized_settlement_active', 'پرداخت جداگانه این حساب شروع شده است؛ برای سفارش تازه لطفاً با صندوق هماهنگ کنید.', 409);
        }
        if (accommodation_transfer_blocks_invoice_edit($sessionId)) {
            throw new GuestOrderException('settlement_pending', 'نتیجه ثبت حساب اقامتگاه هنوز مشخص نشده است؛ لطفاً با همکاران ما هماهنگ کنید.', 409);
        }
        ...
        // A guest device has one mutable draft per live session, whether the table
        // is still pending its first confirmation or already active.
        if ($payload['device_token'] !== '') {
            $pendingDeviceStmt = $pdo->prepare("SELECT public_code,client_token,id FROM orders WHERE session_id=? AND device_token=? AND status IN('pending_approval','new') ORDER BY id DESC LIMIT 1 FOR UPDATE");
            $pendingDeviceStmt->execute([$sessionId, $payload['device_token']]);
            if ($pendingDeviceOrder = $pendingDeviceStmt->fetch()) {
                throw new GuestOrderException(
                    'pending_order_exists',
                    'یک سفارش از همین گوشی هنوز منتظر تأیید است؛ همان سفارش را باز کن و تغییر بده.',
                    409,
                    [
                        'order_code' => (string)$pendingDeviceOrder['public_code'],
                        'client_token' => (string)$pendingDeviceOrder['client_token'],
                        'order_number' => order_display_number($pendingDeviceOrder),
                    ]
                );
            }
        }
    ...
    $requestedIds = array_column($payload['items'], 'id');
    $itemsById = order_catalog_items_locked($pdo, $requestedIds);
    ...
    foreach ($payload['items'] as $line) {
        ...
        if (!order_catalog_item_is_orderable($item, 'guest')) {
            $unavailable[] = $item ? (string)$item['name'] : ('آیتم ' . fa_digits($line['id']));
            continue;
        }
        ...
        if ($line['expected_price'] === null || (int)$line['expected_price'] !== (int)$item['price']) {
    ...
    if ($unavailable) {
        throw new GuestOrderException(
            'items_unavailable',
            customer_message('items_unavailable', ['items' => implode('، ', $unavailable)]),
            409,
            ['items' => $unavailable]
        );
    }
    if ($serviceBlocked) {
        ...
        throw new GuestOrderException(
            'service_unavailable',
            order_acceptance_message($scope),
            409,
            ['items'=>$names,'scope'=>$scope]
        );
    }
    if ($priceChanges) {
        throw new GuestOrderException(
            'prices_changed',
            customer_message('prices_changed'),
            409,
            ['items' => $priceChanges]
        );
    }
    ...
    foreach ($payload['items'] as $line) {
        ...
        if (!order_catalog_item_allows_fulfillment($item,$mode)) {
            throw new GuestOrderException('takeaway_not_allowed','«'.(string)$item['name'].'» فقط داخل کافه قابل سرو است.',409,['item_id'=>(int)$item['id'],'item_name'=>(string)$item['name']]);
        }
    ...
    $insert = $pdo->prepare(
        'INSERT INTO orders(public_code,client_token,device_token,table_id,session_id,status,customer_note,total_amount,business_order_number,business_date,business_shift_key,business_shift_label,business_cutoff_snapshot)
         VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)'
    );
    $insert->execute([
        $publicCode,
        $payload['client_token'],
        $payload['device_token'] !== '' ? $payload['device_token'] : null,
        $tableId,
        $sessionId,
        $orderStatus,
        ...
        $total,
        $businessOrderNumber,
        (string)$business['business_date'],
        (string)$business['shift_key'],
        (string)$business['shift_label'],
        (string)$business['cutoff'],
    ]);
    $orderId = (int)$pdo->lastInsertId();

    $lineStmt = $pdo->prepare(
        'INSERT INTO order_items(order_id,item_id,item_name,unit_price,quantity,ordered_quantity,item_note,fulfillment_mode,preparation_station,line_total)
         VALUES(?,?,?,?,?,?,?,?,?,?)'
    );
    ...
    $pdo->prepare('INSERT INTO order_status_history(order_id,from_status,to_status,actor_user_id) VALUES(?,NULL,?,NULL)')
        ->execute([$orderId, $orderStatus]);
    ...
    json_response(guest_order_response($createdOrder, false));
    ...
    if ($pdo->inTransaction()) $pdo->rollBack();
    error_log('Cafe create_order error: ' . $e->getMessage());
    json_response(['success' => false, 'code' => 'order_failed', 'message' => customer_message('order_failed')], 500);
}
