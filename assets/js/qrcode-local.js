/* QRCode generator based on Kazuhiko Arase's MIT-licensed QRCode for JavaScript. */
(function(global){
  var modules={}; var cache={};
  function define(id, fn){ modules[id]=fn; }
  function req(id){ if(cache[id]) return cache[id].exports; if(!modules[id]) throw new Error('Module '+id+' not found'); var m={exports:{}}; cache[id]=m; modules[id](m,m.exports,req); return m.exports; }
...
module.exports = QRCode;
  ...
  global.CafeQRCode={
    draw:function(canvas,text,size){
      size=size||320;
      var qr=new QRCode(-1,0); qr.addData(String(text)); qr.make();
      var count=qr.getModuleCount(), margin=4, total=count+margin*2;
      canvas.width=size; canvas.height=size;
      var ctx=canvas.getContext('2d'); ctx.fillStyle='#fff'; ctx.fillRect(0,0,size,size); ctx.fillStyle='#000';
      var scale=size/total;
      for(var r=0;r<count;r++) for(var c=0;c<count;c++) if(qr.isDark(r,c)){
        var x=Math.round((c+margin)*scale), y=Math.round((r+margin)*scale);
        var x2=Math.round((c+margin+1)*scale), y2=Math.round((r+margin+1)*scale);
        ctx.fillRect(x,y,x2-x,y2-y);
      }
      return canvas;
    },
    svg:function(text,size){
      size=size||1200;
      var qr=new QRCode(-1,0); qr.addData(String(text)); qr.make();
      var count=qr.getModuleCount(), margin=4, total=count+margin*2;
      var paths=[];
      for(var r=0;r<count;r++) for(var c=0;c<count;c++) if(qr.isDark(r,c)) paths.push('M'+(c+margin)+' '+(r+margin)+'h1v1h-1z');
      return '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 '+total+' '+total+'" width="'+size+'" height="'+size+'" shape-rendering="crispEdges"><rect width="100%" height="100%" fill="#fff"/><path d="'+paths.join('')+'" fill="#000"/></svg>';
    }
  };
})(window);
