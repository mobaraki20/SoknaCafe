(() => {
  ...
  function orderReviewMarkup(order, inline=false) {
    ...
    return `<div class="order-review-lines">${lines}</div>${order.customer_note?`<div class="order-note-box order-review-note"><strong>یادداشت مهمان</strong><span>${esc(order.customer_note)}</span></div>`:''}<div class="order-review-total"><span>جمع سفارش</span><strong>${money(order.total_amount)}</strong></div>${actions}`;
  }
  ...
  function renderTableDetail(table) {
    ...
    const pending=Number(table.unconfirmed_order_count||0);
    const confirmed=(table.bill_orders||[]).filter((o)=>o.status==='accounted');
    const subtotal=Number(table.bill_subtotal||0), discount=Number(table.bill_discount||0), total=Number(table.bill_final_total||0);
    ...
    const invoiceTotals=discount?`<div class="invoice-totals"><div class="invoice-total-row"><span>جمع اقلام</span><strong>${moneyNumber(subtotal)}</strong></div><div class="invoice-total-row is-discount"><span>تخفیف${table.discount_by?` · ${esc(table.discount_by)}`:''}</span><strong>− ${moneyNumber(discount)}</strong></div></div>`:'';
  ...
  function applyItemizedReview(review,items){
    ...
    if(breakdown){
      const discount=Number(review.discount||0);breakdown.textContent=discount>0?`قبل تخفیف ${money(review.subtotal)} · سهم تخفیف ${money(discount)}${review.closes_session?' · آخرین پرداخت':''}`:(review.closes_session?'آخرین پرداخت · حساب کامل می‌شود':'');
  ...
  function prepareSettlementContext(table,{resetPrint=true}={}){
    ...
    $('checkoutModalSummary').textContent=`${paid>0?`جمع ${moneyNumber(table.bill_final_total)} · پرداخت‌شده ${moneyNumber(paid)} · `:''}مانده قابل تسویه: ${moneyNumber(remaining)} تومان${pending?` · ${digits(pending)} اصلاحیه آماده‌سازی هنوز باز است و بعد از تسویه هم باقی می‌ماند.`:''}`;
    $('directSettlementChoiceLabel').textContent=paid>0?'پرداخت تمام مانده':'تسویه مستقیم';
  ...
  function openSubscriber(table) {
    ...
    $('subscriberSearch').value='';$('subscriberSearchResult').innerHTML='<div class="empty-state">نام یا شماره موبایل مشترک را وارد کنید.</div>';
