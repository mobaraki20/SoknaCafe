...
$notice = '';
$error = '';
$printWorkerComponent = print_worker_component_metadata();
...
function printing_queue_readiness_error(array $health,string $queueName,string $role): string
{
    ...
    return match((string)($health['reason']??'')){
        'heartbeat_missing'=>'سرویس چاپ داخلی هنوز Heartbeat معتبر گزارش نکرده است. Setup/Repair و وضعیت سرویس را بررسی کنید.',
        'heartbeat_stale'=>'آخرین Heartbeat سرویس چاپ داخلی قدیمی است؛ وضعیت Runtime را بررسی کنید.',
        'discovery_missing'=>'سرویس چاپ داخلی فعال است، اما هنوز فهرست پرینترهای Windows دریافت نشده است.',
        'discovery_stale'=>'فهرست پرینترهای Windows تازه نیست؛ وضعیت سرویس چاپ داخلی را بررسی کنید.',
        'queue_not_found'=>'پرینتر «'.$queueName.'» در آخرین فهرست Windows پیدا نشد.',
        'queue_offline'=>'پرینتر «'.$queueName.'» آفلاین است.',
        'queue_paused'=>'صف پرینتر «'.$queueName.'» متوقف است.',
        'queue_paper_out'=>'پرینتر «'.$queueName.'» کاغذ ندارد.',
        'queue_error'=>'پرینتر «'.$queueName.'» خطا گزارش می‌کند.',
        'retired'=>'هویت سرویس چاپ داخلی معتبر نیست؛ Repair را اجرا کنید.',
        default=>'پرینتر '.$role.' آماده نیست: '.(string)($health['label']??'وضعیت نامشخص'),
    };
}
...
function printing_agent_health(array $agent): array
{
    ...
    if(!$online)return ['state'=>'unavailable','label'=>'آفلاین','detail'=>'ارتباط تازه‌ای از سرویس چاپ داخلی دریافت نشده','health'=>$health];
    ...
    if($authBlockedReports>0||$reconcileReports>0)return ['state'=>'attention','label'=>'گزارش چاپ نیازمند رسیدگی','detail'=>fa_digits($authBlockedReports+$reconcileReports).' گزارش سرویس چاپ هنوز ACK معتبر نگرفته است','health'=>$health];
    if(!$configOk||!$workerOk||!$lockOk)return ['state'=>'attention','label'=>'نیازمند بررسی','detail'=>'سرویس چاپ داخلی یک خطای داخلی یا پیکربندی گزارش کرده','health'=>$health];
    ...
    if($pendingReports>0)return ['state'=>'degraded','label'=>'گزارش در انتظار ACK','detail'=>fa_digits($pendingReports).' گزارش نتیجه هنوز در صف گزارش سرویس چاپ مانده است','health'=>$health];
    return ['state'=>'healthy','label'=>'آماده','detail'=>'ارتباط و وضعیت سرویس چاپ داخلی عادی است','health'=>$health];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'save_checkout_print_default') {
        ...
        } elseif (in_array($action,['create_agent','rotate_agent','set_agent_active','delete_agent'],true)) {
            throw new RuntimeException('هویت و کلید سرویس چاپ داخلی توسط Setup/Repair خود SOKNA مدیریت می‌شود و از پنل قابل ساخت، تعویض یا حذف نیست.');
            ...
            $fallbackAgent=$locked[$expectedFallback]??null;if(!$fallbackAgent)throw new RuntimeException('سرویس چاپ داخلی برای مسیر جایگزین آماده نیست.');
            ...
            if($expectedPrimary>0&&isset($locked[$expectedPrimary])&&$expectedPrimaryQueue!==''){$oldPrimaryHealth=printing_queue_health($locked[$expectedPrimary],$expectedPrimaryQueue);if($oldPrimaryHealth['ready']){$newFallbackId=$expectedPrimary;$newFallbackQueue=$expectedPrimaryQueue;}}
            ...
            if($queue!==''&&$fallbackQueue!==''&&strcasecmp($queue,$fallbackQueue)===0)throw new RuntimeException('پرینتر اصلی و جایگزین باید متفاوت باشند.');
            $internalAgent=print_internal_worker_agent($pdo,true);
            if(($active||$queue!==''||$fallbackQueue!=='')&&!$internalAgent)throw new RuntimeException('سرویس چاپ داخلی هنوز توسط Setup/Repair آماده نشده است.');
            $agentId=$queue!==''?(int)$internalAgent['id']:0;$fallbackAgentId=$fallbackQueue!==''?(int)$internalAgent['id']:0;
            ...
            if($active&&$queue==='')throw new RuntimeException('برای فعال‌سازی، پرینتر اصلی را انتخاب کنید.');
            ...
            if($queue!==''){$qh=printing_queue_health($internalAgent,$queue);if(!$qh['ready'])throw new RuntimeException(printing_queue_readiness_error($qh,$queue,'اصلی'));}
            if($fallbackQueue!==''){$qh=printing_queue_health($internalAgent,$fallbackQueue);if(!$qh['ready'])throw new RuntimeException(printing_queue_readiness_error($qh,$fallbackQueue,'جایگزین'));}
...
$internalAgent=print_internal_worker_agent($pdo);
$internalAgentId=(int)($internalAgent['id']??0);
$legacyActiveAgents=array_values(array_filter($agents,static fn(array $a): bool=>(int)($a['id']??0)!==$internalAgentId));
...
$printerData=array_values(array_map(static function($p):array{return ['name'=>(string)($p['name']??''),'default'=>(bool)($p['default']??false),'offline'=>(bool)($p['offline']??false),'status'=>(string)($p['status']??'')];},array_filter($internalAgentId>0?($printerLists[$internalAgentId]??[]):[],static fn($p):bool=>is_array($p)&&trim((string)($p['name']??''))!=='')));
$setupComplete=count($requiredDestinations)>0&&$readyRequired===count($requiredDestinations);if($tabInput===''&&!$setupComplete)$tab='settings';$internalOnline=$internalAgent?print_agent_online($internalAgent):false;$agentUpdateCount=$internalAgent&&print_agent_needs_update($internalAgent)?1:0;
...
$humanStatus=static function(array $job): string { return match((string)$job['status']){'submitted'=>'تحویل به صف چاپ Windows','pending'=>'در صف چاپ','blocked'=>'نیازمند تنظیم','reserved'=>'رزرو موقت','claimed'=>'تحویل به سرویس چاپ','failed'=>'خطای پیش از ارسال','unknown'=>'نتیجه نامشخص','recovery_hold'=>'نیاز به بررسی','cancelled'=>'لغوشده',default=>'در حال بررسی'}; };
$attemptState=static function(string $state): string { return match($state){'reserved'=>'رزرو برای رایانه','claimed'=>'دریافت توسط رایانه','started'=>'شروع چاپ','submitted'=>'تحویل به صف چاپ Windows','failed'=>'خطای پیش از ارسال','unknown'=>'نتیجه نامشخص','recovery_hold'=>'نیاز به بررسی','expired'=>'مهلت دریافت پایان یافت',default=>'وضعیت داخلی'}; };
...
<section class="print5-release-strip <?= $agentUpdateCount>0?'has-update':'' ?>">
  <div><span class="print5-status-dot <?= $agentUpdateCount>0?'is-warn':'is-ok' ?>" aria-hidden="true"></span><div><strong>سرویس چاپ داخلی SOKNA <?= e((string)$printWorkerComponent['version']) ?></strong><small><?= $agentUpdateCount>0?fa_digits($agentUpdateCount).' هویت فعال نسخه قدیمی‌تری گزارش کرده است؛ Repair خود SOKNA را اجرا کنید':'این سرویس همراه SOKNA Local نصب و نگهداری می‌شود و برنامه جداگانه‌ای برای دانلود ندارد' ?></small></div></div>
  <div class="actions"><span class="badge <?= $agentUpdateCount>0?'badge-pending':'badge-posted' ?>"><?= $agentUpdateCount>0?'نیازمند Repair':'تحت مدیریت SOKNA Local' ?></span></div>
</section>
...
<section class="print5-status-strip <?= e($overallClass) ?>" aria-label="وضعیت کلی چاپ" data-print-live-status data-snapshot-url="<?= e(asset('api/print_status_snapshot.php')) ?>">
  ...
  <div class="print5-status-facts"><span><b data-print-live-agents><?= $internalOnline?'آماده':'—' ?></b> سرویس چاپ داخلی</span><span><b data-print-live-destinations><?= fa_digits($readyDestinations) ?>/<?= fa_digits(count($destinations)) ?></b> مقصد آماده</span><span class="<?= $problemCount?'is-problem':'' ?>"><b data-print-live-problems><?= fa_digits($problemCount) ?></b> نیازمند رسیدگی</span></div>
  <a class="btn btn-light" href="<?= e(asset('admin/printing.php?tab=overview')) ?>"><?= ui_icon('refresh') ?> تازه‌سازی</a>
</section>

<?php if(!$setupComplete): ?><section class="print5-setup-callout"><div><?= ui_icon('warning') ?><div><strong>راه‌اندازی چاپ کامل نشده است</strong><span>Setup/Repair خود SOKNA باید سرویس چاپ داخلی را آماده کند؛ سپس برای همه مقصدهای فعال، پرینتر ویندوز را انتخاب کنید.</span></div></div><a class="btn btn-primary" href="<?= e(asset('admin/printing.php?tab=settings')) ?>">تکمیل تنظیمات</a></section><?php endif; ?>
...
<div class="print5-overview-grid">
  ...
  <aside class="print5-overview-side">
    <section class="card print5-infra-card"><div class="card-head"><div class="panel-copy-stack"><strong>زیرساخت چاپ</strong><small class="muted">خلاصه سرویس داخلی و مسیرهای چاپ</small></div><a class="panel-text-action" href="<?= e(asset('admin/printing.php?tab=settings')) ?>">مدیریت</a></div><div class="print5-infra-list">
      <?php if(!$internalAgent): ?><div class="empty-state">سرویس چاپ داخلی هنوز توسط Setup/Repair آماده نشده است.</div><?php else: $ah=printing_agent_health($internalAgent); ?><div class="print5-infra-row"><span class="print5-status-dot <?= e(printing_agent_health_class((string)$ah['state'])) ?>"></span><div><strong>سرویس چاپ داخلی</strong><small><?= e((string)($internalAgent['hostname']?:'هنوز متصل نشده')) ?> · نسخه <?= e((string)($internalAgent['agent_version']?:'—')) ?></small></div><span><?= e((string)$ah['label']) ?></span></div><?php endif; ?>
    </div><div class="print5-infra-subhead">مقصدها</div><div class="print5-infra-list"><?php if(!$destinations): ?><div class="empty-state">مقصد چاپی تعریف نشده است.</div><?php else: foreach($destinations as $destination): $route=print_destination_operational_route($pdo,(string)$destination['destination_key']);$routeReady=$route!==null;$routeAgent='سرویس چاپ داخلی';$routeQueue=$routeReady?(string)$route['windows_queue_name']:(string)($destination['windows_queue_name']??'');$routeState=$routeReady?(($route['role']==='fallback'?'آماده · جایگزین':'آماده')):((int)$destination['active']===1?'نیازمند بررسی':'غیرفعال'); ?><div class="print5-infra-row"><span class="print5-status-dot <?= $routeReady?'is-ok':'is-warn' ?>"></span><div><strong><?= e((string)$destination['label']) ?></strong><small><?= e($routeQueue?:'پرینتر انتخاب نشده') ?></small></div><span><?= e($routeState) ?></span></div><?php endforeach; endif; ?></div></section>
    <section class="card print5-side-actions"><a href="<?= e(asset('admin/printing.php?tab=diagnostics')) ?>"><?= ui_icon('operations') ?><div><strong>عیب‌یابی چاپ</strong><span>API، سرویس داخلی، پرینتر و چاپ آزمایشی</span></div><?= ui_icon('chevron-left') ?></a><a href="<?= e(asset('admin/print_templates.php')) ?>"><?= ui_icon('print') ?><div><strong>قالب‌های چاپ</strong><span>پیش‌نمایش و نسخه‌بندی قالب‌ها</span></div><?= ui_icon('chevron-left') ?></a></section>
  </aside>
</div>

<?php elseif($tab==='settings'): ?>
<section class="card print5-section" id="print-agents"><div class="card-head"><div class="panel-copy-stack"><strong>سرویس چاپ داخلی</strong><small class="muted">هویت و کلید اتصال توسط Setup/Repair خود SOKNA مدیریت می‌شود؛ این صفحه فقط وضعیت را نمایش می‌دهد.</small></div></div><div class="card-body">
  <?php if(!$internalAgent): ?><div class="empty-state print5-empty">سرویس چاپ داخلی هنوز در دیتابیس جفت نشده است. Setup/Repair خود SOKNA را اجرا کنید؛ کلید دستی یا نصب برنامه جدا لازم نیست.</div><?php else: $ah=printing_agent_health($internalAgent);$list=$printerLists[$internalAgentId]??[]; ?><div class="print5-agent-list"><article class="print5-agent-row is-settings"><div class="print5-agent-main"><span class="print5-status-dot <?= e(printing_agent_health_class((string)$ah['state'])) ?>"></span><div><strong>سرویس چاپ داخلی سکنا</strong><span><?= e((string)($internalAgent['hostname']?:'هنوز متصل نشده')) ?> · نسخه <?= e((string)($internalAgent['agent_version']?:'—')) ?> · <?= fa_digits(count($list)) ?> پرینتر شناسایی‌شده</span></div></div><span class="badge <?= (string)$ah['state']==='healthy'?'badge-posted':'badge-pending' ?>"><?= e((string)$ah['label']) ?></span></article></div><?php endif; ?>
  <?php if($legacyActiveAgents): ?><div class="alert alert-warning"><?= fa_digits(count($legacyActiveAgents)) ?> هویت چاپ قدیمی هنوز برای حفظ تاریخچه/کارهای ناتمام باقی مانده است. مقصدهای جدید فقط از سرویس چاپ داخلی استفاده می‌کنند؛ پاک‌سازی نهایی بعد از Reconciliation انجام می‌شود.</div><?php endif; ?>
</div></section>

<?php if($retiredAgents): ?><section class="card print5-section" id="print-retired-agents"><div class="card-head"><div class="panel-copy-stack"><strong>آرشیو هویت‌های قدیمی چاپ</strong><small class="muted">این موارد فقط برای تاریخچه نمایش داده می‌شوند و قابل فعال‌سازی، تعویض کلید یا انتخاب در مسیر نیستند.</small></div></div><div class="card-body"><div class="print5-agent-list"><?php foreach($retiredAgents as $agent): ?><article class="print5-agent-row is-settings"><div class="print5-agent-main"><span class="print5-status-dot is-off"></span><div><strong><?= e((string)$agent['name']) ?></strong><span><?= e((string)($agent['hostname']?:'بدون hostname')) ?></span></div></div><div class="print5-agent-facts"><div><small>نسخه آخر</small><strong dir="ltr"><?= e((string)($agent['agent_version']?:'—')) ?></strong></div><div><small>بازنشستگی</small><strong><?= !empty($agent['retired_at'])?e(format_jalali_compact((string)$agent['retired_at'])):'—' ?></strong></div><div><small>آخرین Heartbeat</small><strong><?= !empty($agent['last_heartbeat_at'])?e(format_jalali_compact((string)$agent['last_heartbeat_at'])):'—' ?></strong></div><div><small>آخرین ارتباط</small><strong><?= !empty($agent['last_seen_at'])?e(format_jalali_compact((string)$agent['last_seen_at'])):'—' ?></strong></div></div></article><?php endforeach; ?></div></div></section><?php endif; ?>

<section class="card print5-section" id="print-destinations"><div class="card-head"><div class="panel-copy-stack"><strong>مسیرهای چاپ</strong><small class="muted">حالت عادی فقط خلاصه را نشان می‌دهد؛ برای انتخاب پرینتر اصلی یا جایگزین، همان مقصد را باز کنید.</small></div></div><div class="card-body"><div class="print5-destination-list">
<?php foreach($destinations as $destination):
    $key=(string)$destination['destination_key'];
    $type=(string)$destination['destination_type'];
    $areas=print_destination_areas($destination);
    $currentQueue=(string)($destination['windows_queue_name']??'');
    $fallbackQueue=(string)($destination['fallback_windows_queue_name']??'');
    $operationalRoute=print_destination_operational_route($pdo,$key);
    $destinationJobCount=$destinationJobCounts[$key]??0;
    $summaryQueue=$operationalRoute?(string)$operationalRoute['windows_queue_name']:$currentQueue;
    $state=(int)$destination['active']!==1
        ?'غیرفعال'
        :($operationalRoute
            ?($operationalRoute['role']==='fallback'?'آماده از پرینتر جایگزین':'آماده')
            :($currentQueue===''?'نیازمند انتخاب پرینتر':'پرینتر آماده نیست'));
?>
<details class="print5-destination-editor">
  <summary>
    <div><span class="print5-status-dot <?= $operationalRoute?'is-ok':'is-warn' ?>"></span><div><strong><?= e((string)$destination['label']) ?></strong><small><?= e($summaryQueue?:'پرینتر انتخاب نشده') ?></small></div></div>
    <div><span class="print5-state <?= $operationalRoute?'is-ok':'is-warn' ?>"><?= e($state) ?></span><span class="print5-edit-label">ویرایش <?= ui_icon('chevron-down') ?></span></div>
  </summary>
  <?php if((int)($destination['fallback_agent_id']??0)>0&&$fallbackQueue!==''): ?>
  <form method="post" class="print5-promote-fallback" data-print-mutation data-confirm="پرینتر جایگزین به پرینتر اصلی تبدیل می‌شود. سوابق چاپ قبلی بدون تغییر حفظ می‌شوند و این تغییر فقط برای کارهای جدید است." data-confirm-title="تبدیل پرینتر جایگزین به اصلی؟" data-confirm-ok="تبدیل به اصلی">
    <?= csrf_field() ?>
    <input type="hidden" name="return_tab" value="settings">
    <input type="hidden" name="action" value="promote_fallback">
    <input type="hidden" name="destination_key" value="<?= e($key) ?>">
    <input type="hidden" name="expected_primary_agent_id" value="<?= (int)($destination['agent_id']??0) ?>">
    <input type="hidden" name="expected_primary_queue" value="<?= e($currentQueue) ?>">
    <input type="hidden" name="expected_fallback_agent_id" value="<?= (int)($destination['fallback_agent_id']??0) ?>">
    <input type="hidden" name="expected_fallback_queue" value="<?= e($fallbackQueue) ?>">
    <button class="btn btn-light" type="submit">تبدیل پرینتر جایگزین به اصلی</button>
  </form>
  <?php endif; ?>
  <form method="post" class="print5-destination" data-print-destination>
    <?= csrf_field() ?>
    <input type="hidden" name="return_tab" value="settings">
    <input type="hidden" name="action" value="save_destination">
    <input type="hidden" name="destination_key" value="<?= e($key) ?>">
    <div class="print5-destination-fields">
      <div class="print5-pair" data-print-pair><label><span>پرینتر اصلی</span><select class="form-control" name="windows_queue_name" data-choice-mode="browse" data-print-printer-select="primary" data-current-printer="<?= e($currentQueue) ?>"><option value="">— انتخاب پرینتر —</option></select></label></div>
      <label class="print5-active"><input type="checkbox" name="active" <?= (int)$destination['active']?'checked':'' ?>><span>این مقصد فعال باشد</span></label>
      <label class="print5-active"><input type="checkbox" name="required_for_operation" <?= (int)($destination['required_for_operation']??1)?'checked':'' ?>><span>این مسیر برای سلامت عملیاتی ضروری است</span></label>
    </div>
    <details class="print5-destination-advanced"><summary>تنظیمات بیشتر مقصد</summary><div class="print5-advanced-fields">
      <label><span>نام مقصد</span><input class="form-control" name="label" value="<?= e((string)$destination['label']) ?>" maxlength="160" required></label>
      <?php if($type==='preparation'): ?><fieldset class="print5-area-field"><legend>بخش‌های این مقصد</legend><div class="compact-area-grid"><?php foreach(preparation_operational_areas() as $area=>$areaLabel): ?><label><input type="checkbox" name="preparation_areas[]" value="<?= e($area) ?>" <?= in_array($area,$areas,true)?'checked':'' ?>><span><?= e($areaLabel) ?></span></label><?php endforeach; ?></div></fieldset><?php endif; ?>
      <label><span>پرینتر جایگزین</span><select class="form-control" name="fallback_windows_queue_name" data-choice-mode="browse" data-print-printer-select="fallback" data-current-printer="<?= e($fallbackQueue) ?>"><option value="">بدون پرینتر جایگزین</option></select></label>
      <label><span>عرض رول</span><select class="form-control" name="paper_width_mm" data-choice-mode="compact"><option value="80" <?= (int)$destination['paper_width_mm']===80?'selected':'' ?>>۸۰ میلی‌متر</option><option value="58" <?= (int)$destination['paper_width_mm']===58?'selected':'' ?>>۵۸ میلی‌متر</option></select></label>
      <label><span>عرض قابل چاپ</span><input class="form-control" name="printable_width_mm" inputmode="decimal" enterkeyhint="done" value="<?= e(numeric_input_display_value((string)$destination['printable_width_mm'])) ?>"></label>
      <label><span>تعداد نسخه</span><select class="form-control" name="copies" data-choice-mode="compact"><?php foreach([1,2,3] as $copy): ?><option value="<?= $copy ?>" <?= (int)$destination['copies']===$copy?'selected':'' ?>><?= fa_digits($copy) ?></option><?php endforeach; ?></select></label>
    </div></details>
    <div class="print5-destination-footer"><span class="print5-destination-meta">رول <?= fa_digits((int)$destination['paper_width_mm']) ?> میلی‌متر · <?= fa_digits((int)$destination['copies']) ?> نسخه</span><div class="actions"><button class="btn btn-light" type="button" data-print-editor-close>انصراف</button><button class="btn btn-primary" type="submit">ذخیره تغییرات</button><?php if($type==='preparation'&&!in_array($key,['prep_shared'],true)): ?><?php if($destinationJobCount===0): ?><button class="btn btn-outline-danger" type="submit" name="action" value="delete_destination" form="delete-print-destination-<?= e($key) ?>">حذف مقصد</button><?php else: ?><button class="btn btn-outline-danger" type="button" disabled title="این مقصد سابقه چاپ دارد؛ برای حفظ Audit فقط غیرفعال می‌شود.">حذف مقصد</button><?php endif; ?><?php endif; ?></div></div>
  </form>
  <?php if($type==='preparation'&&!in_array($key,['prep_shared'],true)&&$destinationJobCount===0): ?><form id="delete-print-destination-<?= e($key) ?>" method="post" class="hidden" data-confirm="مقصد «<?= e((string)$destination['label']) ?>» حذف می‌شود. این عمل فقط چون هیچ سابقه چاپی ندارد مجاز است." data-confirm-title="حذف مقصد چاپ؟" data-confirm-ok="حذف مقصد" data-confirm-danger="1"><?= csrf_field() ?><input type="hidden" name="return_tab" value="settings"><input type="hidden" name="action" value="delete_destination"><input type="hidden" name="destination_key" value="<?= e($key) ?>"></form><?php endif; ?>
</details>
...
<?php else: ?>
<section class="print5-diagnostic-intro"><div><?= ui_icon('operations') ?><div><strong>عیب‌یابی فنی چاپ</strong><span>این صفحه وضعیت سرویس چاپ داخلی SOKNA، API، رؤیت‌پذیری پرینتر و مسیر واقعی چاپ آزمایشی را نشان می‌دهد. نصب و Repair سرویس از Setup خود SOKNA انجام می‌شود.</span></div></div><span class="badge badge-posted">نسخه داخلی <?= e(print_agent_recommended_version()) ?></span></section>
<section class="card print5-section"><div class="card-head"><div class="panel-copy-stack"><strong>سلامت سرویس چاپ داخلی</strong><small class="muted">جزئیات فنی فقط اینجا نمایش داده می‌شوند تا نمای روزمره شلوغ نشود.</small></div></div><div class="card-body"><div class="print5-agent-list"><?php if(!$internalAgent): ?><div class="empty-state">سرویس چاپ داخلی هنوز آماده نشده است.</div><?php else: foreach([$internalAgent] as $agent): $ah=printing_agent_health($agent);$health=(array)$ah['health'];$lastAction=trim((string)($health['last_successful_action']??''));$lastApiSuccess=trim((string)($health['last_api_success_at']??''));$lastApiError=trim((string)($health['last_api_error_code']??''));$apiFailures=max(0,(int)($health['consecutive_api_failures']??0));$apiLatency=isset($health['last_api_latency_ms'])?(int)$health['last_api_latency_ms']:null; ?><article class="print5-agent-row is-diagnostic"><div class="print5-agent-main"><span class="print5-status-dot <?= e(printing_agent_health_class((string)$ah['state'])) ?>"></span><div><strong><?= e((string)$agent['name']) ?></strong><span><?= e((string)($agent['hostname']?:'هنوز متصل نشده')) ?> · نسخه <?= e((string)($agent['agent_version']?:'—')) ?></span></div></div><div class="panel-diagnostic-grid <?= in_array((string)$ah['state'],['degraded','attention','unavailable'],true)?'has-warning':'' ?>"><div class="panel-diagnostic-item is-primary"><span class="panel-diagnostic-label">وضعیت عملیاتی</span><strong><?= e((string)$ah['label']) ?></strong><small><?= e((string)$ah['detail']) ?></small></div><div class="panel-diagnostic-item"><span class="panel-diagnostic-label">آخرین ارتباط API موفق</span><strong class="panel-diagnostic-value-small"><?= $lastApiSuccess!==''?e($lastApiSuccess):'—' ?></strong><small><?= $lastAction!==''?'عملیات: '.e($lastAction):'هنوز گزارش نشده' ?></small></div><div class="panel-diagnostic-item"><span class="panel-diagnostic-label">ارتباط سرویس</span><strong><?= $apiFailures>0?fa_digits($apiFailures).' خطای متوالی':'بدون خطای متوالی' ?></strong><small><?= $lastApiError!==''?'کد: '.e($lastApiError):($apiLatency!==null?'زمان پاسخ: '.fa_digits($apiLatency).' ms':'جزئیات در Heartbeat بعدی') ?></small></div></div></article><?php endforeach; endif; ?></div></div></section>
<section class="card print5-section"><div class="card-head"><div class="panel-copy-stack"><strong>تست مسیرهای چاپ</strong><small class="muted">چاپ آزمایشی از Print API واقعی عبور می‌کند؛ تست مستقیم Windows جای این آزمون را نمی‌گیرد.</small></div></div><div class="card-body"><div class="print5-test-list"><?php foreach($destinations as $destination): $route=print_destination_operational_route($pdo,(string)$destination['destination_key']);$routeReady=$route!==null;$routeAgent='سرویس چاپ داخلی';$routeQueue=$routeReady?(string)$route['windows_queue_name']:(string)($destination['windows_queue_name']??'');$routeLabel=$routeReady?(($route['role']==='fallback'?'آماده از پرینتر جایگزین':'آماده')):'پرینتر آماده نیست'; ?><article class="print5-test-row"><div><span class="print5-status-dot <?= $routeReady?'is-ok':'is-warn' ?>"></span><div><strong><?= e((string)$destination['label']) ?></strong><small><?= e($routeQueue?:'پرینتر انتخاب نشده') ?> · <?= e($routeLabel) ?></small></div></div><form method="post"><?= csrf_field() ?><input type="hidden" name="return_tab" value="diagnostics"><input type="hidden" name="action" value="test_print"><input type="hidden" name="destination_key" value="<?= e((string)$destination['destination_key']) ?>"><button class="btn btn-light" <?= !$routeReady?'disabled':'' ?>>چاپ آزمایشی</button></form></article><?php endforeach; ?></div></div></section>
<?php endif; ?>
