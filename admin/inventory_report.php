...
try{
    ...
    $netSales=(int)(inventory_report_scalar($pdo,"SELECT COALESCE(SUM(sr.total),0) FROM settlement_records sr WHERE $validSettlement AND sr.business_date>=? AND sr.business_date<=?",$params)??0);
    $itemRevenue=(int)(inventory_report_scalar($pdo,"SELECT COALESCE(SUM(sale.line_total),0) FROM settlement_records sr JOIN $saleLineSource ON sale.settlement_id=sr.id WHERE $validSettlement AND sr.business_date>=? AND sr.business_date<=?",$params)??0);
    ...
    $coveredRevenue=(int)(inventory_report_scalar($pdo,"SELECT COALESCE(SUM(CASE WHEN c.component_count>0 AND c.unknown_components=0 THEN sale.line_total ELSE 0 END),0) FROM settlement_records sr JOIN $saleLineSource ON sale.settlement_id=sr.id JOIN order_items oi ON oi.id=sale.order_item_id LEFT JOIN ($costSql) c ON CAST(c.order_item_id AS UNSIGNED)=oi.id WHERE $validSettlement AND sr.business_date>=? AND sr.business_date<=?",$params)??0);
    ...
    $itemSql="SELECT MAX(sale.item_name) item_name,SUM(sale.quantity) qty,SUM(sale.line_total) revenue,COALESCE(SUM(ROUND(c.known_cogs*sale.quantity/NULLIF(oi.quantity,0))),0) known_cogs,
        SUM(CASE WHEN c.component_count IS NULL OR c.component_count=0 OR c.unknown_components>0 THEN sale.line_total ELSE 0 END) uncovered_revenue
...
if(($_GET['action']??'')==='export'){
    ...
    $summary=[
        ...
        ['فروش خالص کل',xlsx_cell($netSales,'money'),'کل فروش تسویه‌شده معتبر در بازه'],
    ...
    $menuRows=[[xlsx_cell('سود تقریبی آیتم‌های منو','title',5)],[xlsx_cell('درآمد آیتم قبل از تخصیص تخفیف فاکتور است؛ برای مقایسه آیتم‌ها استفاده شود.','meta',5)],[xlsx_cell('آیتم','header'),xlsx_cell('تعداد','header'),xlsx_cell('درآمد','header'),xlsx_cell('هزینه مواد','header'),xlsx_cell('سود ناخالص','header')]];
