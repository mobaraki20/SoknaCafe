...
if($_SERVER['REQUEST_METHOD']==='POST'){
 ...
 try{
  ...
  $sum=$pdo->prepare("SELECT SUM(status='completed') invoice_count,SUM(status='reversal') reversal_count,COALESCE(SUM(CASE WHEN status='completed' THEN total-tax_amount WHEN status='reversal' THEN -(total-tax_amount) ELSE 0 END),0) sales_amount,COALESCE(SUM(CASE WHEN status='completed' THEN tax_amount WHEN status='reversal' THEN -tax_amount ELSE 0 END),0) tax_amount,COALESCE(SUM(CASE WHEN status='completed' THEN total WHEN status='reversal' THEN -total ELSE 0 END),0) total_amount,COALESCE(SUM(CASE WHEN status='completed' THEN discount WHEN status='reversal' THEN -discount ELSE 0 END),0) discount_amount,COALESCE(SUM(status='completed' AND destination='direct'),0) direct_count,COALESCE(SUM(status='completed' AND destination='accommodation'),0) accommodation_count,COALESCE(SUM(status='completed' AND destination='subscriber'),0) subscriber_count FROM settlement_records WHERE financial_period_id=?");$sum->execute([$id]);$summary=$sum->fetch()?:[];
  $summary['general_expenses']=expense_period_summary($pdo,$id);
...
$periods=$pdo->query("SELECT fp.*,COALESCE(s.invoice_count,0) invoice_count,COALESCE(s.reversal_count,0) reversal_count,COALESCE(s.sales_amount,0) sales_amount,COALESCE(s.tax_amount,0) tax_amount,COALESCE(s.total_amount,0) total_amount,COALESCE(s.discount_amount,0) discount_amount,COALESCE(ex.expense_count,0) expense_count,COALESCE(ex.expense_reversal_count,0) expense_reversal_count,COALESCE(ex.expense_net_amount,0) expense_net_amount FROM financial_periods fp LEFT JOIN (SELECT financial_period_id,SUM(status='completed') invoice_count,SUM(status='reversal') reversal_count,SUM(CASE WHEN status='completed' THEN total-tax_amount WHEN status='reversal' THEN -(total-tax_amount) ELSE 0 END) sales_amount,SUM(CASE WHEN status='completed' THEN tax_amount WHEN status='reversal' THEN -tax_amount ELSE 0 END) tax_amount,SUM(CASE WHEN status='completed' THEN total WHEN status='reversal' THEN -total ELSE 0 END) total_amount,SUM(CASE WHEN status='completed' THEN discount WHEN status='reversal' THEN -discount ELSE 0 END) discount_amount FROM settlement_records GROUP BY financial_period_id)s ON s.financial_period_id=fp.id LEFT JOIN (SELECT financial_period_id,SUM(status='committed') expense_count,SUM(status='reversal') expense_reversal_count,SUM(CASE WHEN status='committed' THEN amount WHEN status='reversal' THEN -amount ELSE 0 END) expense_net_amount FROM expenses GROUP BY financial_period_id) ex ON ex.financial_period_id=fp.id ORDER BY fp.start_date DESC")->fetchAll();
...
<div class="financial-workspace financial-period-workspace panel-page-flow" data-visual-quality-page="financial_periods">
  <section class="card financial-period-current financial-record-hero">
    ...
    <div class="financial-metric-strip" aria-label="خلاصه دوره جاری">
      <div><span>فروش پس از تخفیف</span><strong><?= e(toman((int)($currentRow['sales_amount']??0))) ?></strong></div><div><span>مالیات</span><strong><?= e(toman((int)($currentRow['tax_amount']??0))) ?></strong></div><div><span>دریافتی نهایی</span><strong><?= e(toman((int)($currentRow['total_amount']??0))) ?></strong></div>
      ...
      <div><span>هزینه‌های عمومی</span><strong><?= e(toman((int)($currentRow['expense_net_amount']??0))) ?></strong></div>
    </div>
  </section>
    ...
    <div class="financial-period-list financial-list">
      ...
      <article class="financial-period-row financial-row is-navigable" data-row-href="invoices.php?period=<?= (int)$period['id'] ?>" role="link" tabindex="0" aria-label="مشاهده فاکتورهای <?= e((string)$period['title']) ?>">
        <div class="financial-row-main">
          ...
          <small><?= fa_digits((int)$period['invoice_count']) ?> فاکتور<?php if((int)$period['reversal_count']>0): ?> · <?= fa_digits((int)$period['reversal_count']) ?> برگشت<?php endif; ?> · هزینه عمومی <?= e(toman((int)$period['expense_net_amount'])) ?><?php if($isOpen): ?> · <span class="text-warning">نیازمند بستن</span><?php endif; ?></small>
        </div>
        <div class="financial-row-amount"><strong><?= e(toman((int)$period['sales_amount'])) ?></strong><small>فروش پیش از مالیات</small><?php if((int)$period['tax_amount']!==0): ?><small>مالیات <?= e(toman((int)$period['tax_amount'])) ?> · دریافتی <?= e(toman((int)$period['total_amount'])) ?></small><?php endif; ?></div>
