...
$pdo=public_db(); $stmt=$pdo->prepare('SELECT p.projection_id,p.password_hash,p.capabilities_json FROM auth_projections p JOIN installations i ON i.installation_id=p.installation_id WHERE p.installation_id=? AND p.username=? AND p.active=1 AND i.active=1 AND i.remote_enabled=1 LIMIT 1'); $stmt->execute([$installationId,$username]); $row=$stmt->fetch();
...
public_json(['ok'=>true,'token'=>$token,'expires_in'=>$ttl,'projection_id'=>(string)$row['projection_id'],'capabilities'=>json_decode((string)$row['capabilities_json'],true)?:[]]);
