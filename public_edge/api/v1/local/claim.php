...
$pdo->beginTransaction(); try{
  $pdo->prepare("UPDATE realtime_requests SET state='expired',lease_token_hash=NULL,lease_expires_at=NULL WHERE installation_id=? AND state IN ('queued','claimed') AND expires_at<=UTC_TIMESTAMP()")->execute([$installationId]);
  $stmt=$pdo->prepare("SELECT id,envelope_json FROM realtime_requests WHERE installation_id=? AND expires_at>UTC_TIMESTAMP() AND (state='queued' OR (state='claimed' AND lease_expires_at<UTC_TIMESTAMP())) ORDER BY id ASC LIMIT 1 FOR UPDATE"); $stmt->execute([$installationId]); $row=$stmt->fetch();
