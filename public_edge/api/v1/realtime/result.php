...
$stmt=public_db()->prepare('SELECT state,result_json,error_code,updated_at FROM realtime_requests WHERE installation_id=? AND request_id=? LIMIT 1'); $stmt->execute([(string)$session['installation_id'],$requestId]); $row=$stmt->fetch(); if(!$row) public_json(['ok'=>false,'error'=>'not_found'],404);
