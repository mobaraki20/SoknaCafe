...
f=t('includes/functions.php'); users=t('admin/users.php'); purchases=t('admin/purchases.php'); schema=t('database/schema.sql'); supply=t('modules/Supply/domain.php'); need_page=t('operator/supply-needs.php'); need_js=t('assets/js/supply-needs.js')
...
updater=t('includes/updater_engine/1.5.2/console.php'); runtime=t('includes/updater_engine/1.5.2/runtime.php'); layout=t('includes/panel_layout.php')
...
# Current Supply model: staff can report needs without purchase authority; purchase/receive is one workflow.
...
for table in ['inventory_supply_needs','inventory_supply_receipts']:
...
need('ثبت تحویل و ورود انبار' in purchases and 'inventory_record_movement_locked' in supply,'direct receive-to-ledger workflow missing')
# Multi-item Supply request is a structural workflow contract, not a UI-copy contract.
for marker in ['id="supplyNeedForm"','id="supplyDraftList"','id="supplySubmit"','ثبت درخواست خرید']:
    need(marker in need_page,f'multi-item supply request marker missing: {marker}')
for field in ['inventory_item_id[]','quantity_major[]']:
    need(field in need_js,f'multi-item supply request field missing: {field}')
need('if($count>20)' in need_page and 'supply_request_upsert_locked' in need_page,'multi-item supply request processing contract missing')
...
need("const SOKNA_UPDATER_RUNTIME_VERSION = '1.5.2';" in runtime,'updater runtime identity not 1.5.2')
print('Operations/permissions/supply contracts PASS.')
