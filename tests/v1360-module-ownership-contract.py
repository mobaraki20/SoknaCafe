...
need(len(registry) == 16, f"unexpected module count: {len(registry)}")
need([key for key,module in registry.items() if module.get('toggleable')] == ['inventory','supply','tax','marketing','reporting','personnel'], 'only end-to-end hardened optional modules may expose runtime toggles')
...
need(len(discovered) == 71, f"expected 71 current schema tables, found {len(discovered)}")
