...
function updater_ensure_storage(): void
{
    ...
    if (!is_dir($storage) && !mkdir($storage, 0750, true) && !is_dir($storage)) throw new RuntimeException('پوشه نگهداری امن ساخته نشد.');
...
function updater_disk_check(int $estimatedBytes = 0): void
{
    ...
    if ($free !== false && $free < $required) throw new RuntimeException('فضای آزاد سرور برای نقطه بازیابی و نصب امن کافی نیست.');
}
...
function updater_restore_dir(): string
{
    ...
    if (!is_dir($dir) && !mkdir($dir, 0750, true) && !is_dir($dir)) throw new RuntimeException('پوشه نقطه بازیابی ساخته نشد.');
    updater_guard_directory($dir);
    return $dir;
}
...
function updater_validate_recovery_runtime(): void
{
    foreach (updater_runtime_paths() as $relative) {
        ...
        if (!$lint['ok']) {
            throw new RuntimeException('کد موتور مستقل به‌روزرسانی معتبر نیست: ' . $relative . ' — ' . $lint['message']);
        }
    }
...
function updater_validate_manifest(array $manifest): array
{
    ...
    if ($runtime === '' || version_compare(SOKNA_UPDATER_RUNTIME_VERSION, $runtime, '<')) throw new RuntimeException('این بسته به موتور به‌روزرسانی نسخه ' . ($runtime ?: 'جدیدتر') . ' نیاز دارد.');
    ...
    if (!is_array($extensions)) throw new RuntimeException('فهرست پیش‌نیازهای PHP معتبر نیست.');
    ...
    foreach ($extensions as $extension) {
        ...
        if (!$skipExtensionCheck && $extension !== '' && !extension_loaded($extension)) throw new RuntimeException('پیش‌نیاز PHP نصب نیست: ' . $extension);
    }
    ...
    foreach ($files as $file) {
        if (!is_array($file)) throw new RuntimeException('ساختار یکی از فایل‌های بسته معتبر نیست.');
        $path=updater_normalize_path((string)($file['path']??''));
        if (isset($normalized[$path])) throw new RuntimeException('مسیر تکراری در فهرست بسته: ' . $path);
        $sha=strtolower(trim((string)($file['sha256']??''))); if(!preg_match('/^[a-f0-9]{64}$/',$sha)) throw new RuntimeException('اعتبار فایل معتبر نیست: '.$path);
    ...
    if ($engineVersion !== '') {
        if (!preg_match('/^\d+\.\d+\.\d+$/', $engineVersion)) throw new RuntimeException('نسخه موتور به‌روزرسانی معتبر نیست.');
    ...
    if ($migration!==null) {
        if(!is_array($migration)) throw new RuntimeException('تعریف تغییرات پایگاه داده معتبر نیست.');
        $migrationPath=trim(str_replace('\\','/',(string)($migration['path']??'')),'/');
        if(!preg_match('#^migrations/[A-Za-z0-9._-]+\.sql$#',$migrationPath)) throw new RuntimeException('مسیر فایل تغییرات پایگاه داده معتبر نیست.');
        $migrationSha=strtolower(trim((string)($migration['sha256']??''))); if(!preg_match('/^[a-f0-9]{64}$/',$migrationSha)) throw new RuntimeException('اعتبار فایل تغییرات پایگاه داده معتبر نیست.');
...
function updater_php_lint(string $path): array
{
    ...
    try {
        ...
        return ['checked' => false, 'ok' => true, 'message' => 'بررسی ساختار داخلی PHP در این سرور در دسترس نبود؛ اعتبار فایل‌ها همچنان بررسی شد.'];
    }
}
...
function updater_validate_package(string $packagePath, bool $stage = false, ?string $stageDir = null): array
{
    ...
    if (strlen($manifestRaw) > 1024 * 1024) throw new RuntimeException('فهرست بسته بیش از حد بزرگ است.');
    try {
        $decodedManifest = json_decode($manifestRaw, true, 128, JSON_THROW_ON_ERROR);
        if (!is_array($decodedManifest)) throw new RuntimeException('فهرست بسته ساختار معتبر ندارد.');
        $manifest = updater_validate_manifest($decodedManifest);
    } catch (JsonException $e) {
        throw new RuntimeException('فهرست بسته معتبر نیست.', 0, $e);
    }
    ...
    foreach ($manifest['files'] as $file) {
        ...
        if ($entries[$entry]['size'] !== $file['size']) throw new RuntimeException('اندازه فایل با فهرست بسته هماهنگ نیست: ' . $file['path']);
        $actual = hash_file('sha256', updater_archive_file_path($packagePath, $entry));
        if (!$actual || !hash_equals($file['sha256'], $actual)) throw new RuntimeException('اعتبار فایل تأیید نشد: ' . $file['path']);
        $total += $file['size'];
        ...
        if ($stage) {
            ...
            if (!$lint['ok']) throw new RuntimeException('خطای ساختاری در ' . $file['path'] . ': ' . $lint['message']);
    ...
    if (trim((string)file_get_contents(updater_archive_file_path($packagePath, 'files/VERSION.txt'))) !== $manifest['version']) {
        throw new RuntimeException('نسخه ثبت‌شده در بسته با نسخه اعلام‌شده یکسان نیست.');
    }
    if ($manifest['migration']) {
        $entry = $manifest['migration']['path'];
        $allowedEntries[$entry] = true;
        if (!isset($entries[$entry])) throw new RuntimeException('فایل تغییرات پایگاه داده در بسته وجود ندارد.');
        $actual = hash_file('sha256', updater_archive_file_path($packagePath, $entry));
        if (!$actual || !hash_equals($manifest['migration']['sha256'], $actual)) throw new RuntimeException('اعتبار فایل تغییرات پایگاه داده تأیید نشد.');
        if ($stage) {
            $target = rtrim($stageDir, '/') . '/' . $entry;
            if (!is_dir(dirname($target)) && !mkdir(dirname($target), 0750, true) && !is_dir(dirname($target))) throw new RuntimeException('پوشه موقت تغییرات پایگاه داده ساخته نشد.');
            if (!copy(updater_archive_file_path($packagePath, $entry), $target)) throw new RuntimeException('آماده‌سازی تغییرات پایگاه داده انجام نشد.');
        }
    }
...
function updater_start(string $pendingId, int $actorUserId): array
{
    ...
    if (!is_dir(dirname($restorePath)) && !mkdir(dirname($restorePath), 0750, true) && !is_dir(dirname($restorePath))) {
        throw new RuntimeException('پوشه نقطه بازیابی ساخته نشد.');
    }
    if (!mkdir($restorePath, 0750, true) && !is_dir($restorePath)) throw new RuntimeException('نقطه بازیابی ساخته نشد.');
    $deny = "Options -Indexes\n<IfModule mod_authz_core.c>\nRequire all denied\n</IfModule>\n<IfModule !mod_authz_core.c>\nDeny from all\n</IfModule>\n";
    ...
    $job = [
        ...
        'message' => 'نقطه بازیابی فایل‌های درگیر ساخته می‌شود؛ سامانه هنوز فعال است.',
        'progress' => 2,
        'manifest' => $manifest,
...
function sru_db_dump_file(PDO $pdo, string $path): array
{
    ...
    if (!$stream) throw new RuntimeException('فایل نقطه بازیابی دیتابیس ساخته نشد.');
    ...
    $write = static function (string $chunk) use ($stream, $hash, &$bytes): void {
        ...
        while ($offset < $length) {
            ...
            if ($written === false || $written === 0) throw new RuntimeException('نوشتن نقطه بازیابی دیتابیس کامل نشد.');
            $offset += $written;
        }
...
{
    if (!is_file($path)) throw new RuntimeException('نقطه بازیابی دیتابیس پیدا نشد.');
    $sql = (string)file_get_contents($path);
    $statements = sru_sql_statements($sql);
    if (!$statements) throw new RuntimeException('نقطه بازیابی دیتابیس خالی است.');
...
function sru_restore_path(array $job): string
{
    ...
    if (!$base || !$parent || $base !== $parent) throw new RuntimeException('مسیر نقطه بازیابی معتبر نیست.');
    return $path;
}
...
function sru_snapshot_file(array &$job, string $relative): void
{
    ...
    if ($exists) {
        ...
        if (!is_dir(dirname($restore)) && !mkdir(dirname($restore), 0750, true) && !is_dir(dirname($restore))) {
            throw new RuntimeException('پوشه نقطه بازیابی فایل ساخته نشد.');
        }
...
function sru_valid_restore_source(string $path): string
{
    ...
    if (!$base || !$parent || $base !== $parent || !$resolved || $resolved !== $path) {
        throw new RuntimeException('مسیر نقطه بازیابی معتبر نیست.');
    }
    return $path;
}
...
function sru_restore_manifest_write(array $job): void
{
    ...
    if (file_put_contents($restore . '/restore-point.json', $json, LOCK_EX) === false) throw new RuntimeException('ثبت نقطه بازیابی انجام نشد.');
}
...
function sru_retry_recovery(array $job): array
{
    try {
        ...
        if ($stage === 'recovery_files') {
            ...
            if ($done) { $job['stage']='recovery_database'; $job['message']='نقطه بازیابی دیتابیس بررسی می‌شود.'; }
            sru_job_write($job);
            return $job;
        }
        if ($stage === 'recovery_database') {
            if (($job['database_ready'] ?? false) && !($job['recovery_database_done'] ?? false)) {
                ...
                if (!$actual || !hash_equals((string)($job['database_sha256'] ?? ''), $actual)) throw new RuntimeException('اعتبار پایگاه داده نقطه بازیابی تأیید نشد.');
    ...
    switch ((string)$job['stage']) {
        case 'snapshot_files':
            ...
            if ($index >= count($allPaths)) {
                ...
                $job['message'] = 'عملیات نوشتنی متوقف و نقطه بازیابی دیتابیس ساخته می‌شود.';
                $job['progress'] = 20;
            } else {
                $job['message'] = 'ساخت نقطه بازیابی فایل‌ها: ' . $index . ' از ' . count($allPaths);
        ...
        case 'snapshot_database':
            ...
            if (is_array($manifest['migration'] ?? null)) {
                ...
                $job['message'] = 'نقطه بازیابی دیتابیس ساخته شد؛ فایل‌های نسخه جدید نصب می‌شوند.';
            } else {
                $job['database_sha256'] = '';
        ...
        case 'migration':
            ...
            if (is_array($migration)) {
                ...
                if (!$actual || !hash_equals((string)$migration['sha256'], $actual)) throw new RuntimeException('فایل آماده‌شده تغییرات پایگاه داده معتبر نیست.');
                $sql = (string)file_get_contents($path);
...
function sru_step_rollback(array $job): array
{
    ...
    if (!$base || !$parent || $base !== $parent || !is_file($source . '/restore-point.json')) throw new RuntimeException('نقطه بازیابی انتخاب‌شده معتبر نیست.');
    ...
    switch ((string)$job['stage']) {
        ...
        case 'rollback_snapshot_database':
            ...
            if (($manifest['migration_applied'] ?? false) === true) {
                ...
                $job['database_sha256'] = '';
                $job['database_ready'] = false;
                $job['message'] = 'این نسخه تغییری در پایگاه داده نداشته است؛ فقط فایل‌ها بازگردانده می‌شوند و داده‌های جاری حفظ می‌شوند.';
            }
        ...
        case 'rollback_database':
            if (($manifest['migration_applied'] ?? false) === true && !($job['rollback_database_done'] ?? false)) {
                ...
                $actual = is_file($dbPath) ? hash_file('sha256', $dbPath) : false;
                if (!$actual || !hash_equals((string)$manifest['database_sha256'], $actual)) throw new RuntimeException('دیتابیس نقطه بازیابی معتبر نیست.');
                sru_db_restore(sru_db(), $dbPath);
...
{
    if (!preg_match('/^[a-f0-9]{24}$/', $restoreId)) throw new RuntimeException('شناسه نقطه بازیابی معتبر نیست.');
    $source = sru_restore_dir() . '/restore-' . $restoreId;
    if (!is_file($source . '/restore-point.json')) throw new RuntimeException('نقطه بازیابی پیدا نشد.');
    $manifest = json_decode((string)file_get_contents($source . '/restore-point.json'), true);
    if (!is_array($manifest)) throw new RuntimeException('نقطه بازیابی معتبر نیست.');
    $id = bin2hex(random_bytes(12));
    $safetyId = bin2hex(random_bytes(12));
    $safetyPath = sru_restore_dir() . '/restore-' . $safetyId;
    if (!mkdir($safetyPath, 0750, true) && !is_dir($safetyPath)) throw new RuntimeException('نقطه بازیابی ایمنی ساخته نشد.');
    $job = [
        'format' => SOKNA_UPDATER_JOB_FORMAT,
        'id' => $id,
        'mode' => 'rollback',
...
function sur_data_validate_emergency(array $state): array
{
    ...
    foreach ($entries as $entry) {
        ...
        if (!$hash || !hash_equals((string)($entry['sha256'] ?? ''), $hash)) throw new RuntimeException('اعتبار پشتیبان اضطراری تأیید نشد: ' . $ep);
    }
