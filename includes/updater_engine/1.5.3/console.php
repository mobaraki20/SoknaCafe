...
if (!defined('SOKNA_UPDATER_ROOT') || !defined('SOKNA_UPDATER_ENGINE_DIR')) {
    ...
    exit('راه‌انداز مرکز به‌روزرسانی در دسترس نیست.');
}
...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET')==='POST') {
    ...
    try {
        ...
        if($action==='legacy_reset'){
            ...
            uu_flash('success','قفل و عملیات‌های قدیمی به‌روزرسانی بایگانی و سامانه آزاد شد. اطلاعات و دیتابیس تغییر نکردند.');uu_redirect();
        }
        ...
        if($action==='start_update'){
            if(!isset($_POST['confirm']))throw new RuntimeException('تأیید ساخت نقطه بازیابی لازم است.');
...
<?php if(!$historyWriteReady): ?><div class="notice warn"><strong>ثبت تاریخچه در دسترس نیست.</strong><br>عملیات را شروع نکنید تا دسترسی نوشتن پوشه به‌روزرسانی اصلاح شود؛ خود موتور خطا را در گزارش فنی بدون اطلاعات حساس ثبت می‌کند.</div><?php endif; ?>
...
<?php elseif($publicStatus==='completed'): ?><section class="card"><div class="pad"><div class="job-status"><div><small class="muted"><?= ($public['mode']??'update')==='rollback'?'بازگشت انجام شد':'آخرین به‌روزرسانی' ?></small><h2 class="version-flow"><span dir="ltr"><?= uu_h((string)$public['from_version']) ?></span><span class="to">به</span><span dir="ltr"><?= uu_h((string)$public['to_version']) ?></span></h2></div><span class="badge">تکمیل‌شده</span></div><div class="notice success">عملیات و بررسی سلامت پایه با موفقیت پایان یافت. نقطه بازگشت قبلی طبق سیاست نگهداری موتور حفظ می‌شود.</div><?php if(!empty($public['migration_required'])): ?><div class="notice <?= !empty($public['migration_applied'])?'success':'warn' ?>"><strong>تغییر دیتابیس:</strong> <?= !empty($public['migration_applied'])?'اعمال شد.':'برای این عملیات ثبت نشده است.' ?><?php if(!empty($public['schema_health']['checked'])): ?> <span>ساختار حیاتی دیتابیس نیز <?= !empty($public['schema_health']['ok'])?'تأیید شد.':'نیازمند بررسی است.' ?></span><?php endif; ?></div><?php elseif(!empty($public['schema_health']['checked'])): ?><div class="notice success">ساختار حیاتی دیتابیس با نسخه نصب‌شده تطبیق دارد.</div><?php endif; ?><?php if($canReturnToPanel): ?><div class="row" style="margin-top:12px"><a class="btn primary" href="../maintenance.php">بازگشت به پشتیبان‌گیری و بازیابی</a></div><?php endif; ?></div></section>
