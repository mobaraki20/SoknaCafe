...
function sokna_module_registry(): array
{
    ...
    $registry = [
        ...
        'orders' => [
            ...
            'reads_from' => ['finance','inventory','printing','notifications','tax'],
            ...
            'entrypoints' => [
                'api/create_order.php','api/order_quote.php','api/guest_orders.php','api/order_status.php','api/table_context.php','api/waiter_call.php',
        ...
        'finance' => [
            ...
            'reads_from' => ['subscribers','accommodation','printing','tax'],
        ...
        'supply' => [
            ...
            'entrypoints' => ['operator/supply-needs.php','admin/purchases.php','admin/purchases_batch.php','assets/js/supply-needs.js','assets/js/supply-purchases.js'],
            ...
            'public_contracts' => [
                ...
                'supply_receive_preparing_locked()','supply_receive_batch_locked()','supply_purchase_groups()','supply_purchase_attention_count()',
            ],
            'manager' => [
                'icon' => 'cart',
                'context' => 'خرید روزانه',
        ...
        'expenses' => [
            ...
            'depends_on' => ['platform','finance'],
            'reads_from' => ['finance'],
            'owner' => 'includes/expenses.php + admin/expenses.php',
            'owns_tables' => ['expense_categories','expenses'],
            'entrypoints' => ['admin/expenses.php'],
            'capabilities' => [],
            'background_jobs' => [],
            'public_contracts' => ['expense_create_locked()','expense_reverse_locked()','expense_correct_locked()','expense category/read-summary contracts'],
        ],
        'tax' => [
            'label' => 'مالیات',
            'type' => 'business',
            'required' => false,
            'toggleable' => true,
            'setting_key' => 'module.tax.enabled',
            'default_enabled' => false,
            'depends_on' => ['platform','menu'],
            'runtime_ready' => 'tax_module_runtime_ready',
            'lifecycle' => [
                'before_enable'=>'tax_module_before_enable_locked',
                'before_disable'=>'tax_module_before_disable_locked',
                'disable_preflight'=>'tax_module_disable_preflight',
            ],
            'reads_from' => ['orders','finance'],
            'owner' => 'includes/tax.php + admin/tax.php',
            'owns_tables' => ['tax_rate_versions','tax_item_policy_versions'],
            'entrypoints' => ['admin/tax.php'],
            'capabilities' => [],
            'background_jobs' => [],
            'public_contracts' => ['tax_order_line_snapshot()','tax_calculate_invoice_lines()','effective-dated tax rate/item policy contracts'],
            'manager' => [
                'icon' => 'receipt',
                'context' => 'مالی',
                'description' => 'مالیات فروش را با نرخ‌های تاریخ‌دار و سیاست هر کالا کنترل می‌کند.',
                'active_note' => 'مالیات روشن است؛ نرخ مؤثر و استثناهای کالا از صفحه مالیات مدیریت می‌شوند.',
                'waiting_note' => 'مالیات روشن شده است اما هنوز نرخ مؤثر ندارد؛ تا ثبت نرخ، مبلغی به سفارش‌ها افزوده نمی‌شود.',
                'disabled_note' => 'مالیات خاموش است و قیمت‌های منو بدون مالیات به حساب منتقل می‌شوند.',
                'impacts' => [
                    'قیمت ثبت‌شده منو همیشه مبلغ پیش از مالیات باقی می‌ماند.',
                    'پس از فعال‌شدن، نرخ هر قلم هنگام ثبت سفارش به‌صورت ثابت در سند ذخیره می‌شود و تاریخچه بعداً بازنویسی نمی‌شود.',
                    'خاموش یا روشن‌کردن مالیات هنگام وجود حساب باز مجاز نیست.',
                ],
                'links' => [
                    ['label'=>'تنظیم مالیات','href'=>'tax.php'],
                ],
            ],
        ],
        'marketing' => [
        ...
        'reporting' => [
            ...
            'default_enabled' => true,
            'depends_on' => ['platform'],
            'reads_from' => ['menu','orders','finance','inventory','accommodation','tax'],
        ...
        'personnel' => [
            ...
            'depends_on' => ['platform'],
            'reads_from' => ['platform'],
            'owner' => 'includes/sokna_center.php + includes/sokna_center_projection.php + admin/personnel.php',
            'owns_tables' => [],
            ...
            'capabilities' => [],
            'background_jobs' => ['SOKNA Runtime -> tools/center-projection-worker.php'],
            'public_contracts' => ['Sokna Center signed handoff','personnel entitlement cache','payroll reminder count read','outbound Cafe user projection v1','legacy inbound user-directory compatibility endpoint','center_return.php remains a safe recovery redirect while disabled'],
            'manager' => [
                'icon' => 'users',
        ...
        'printing' => [
            ...
            'owner' => 'includes/printing.php + print-agent/v4/ + runtime/print-worker/source/ + tools/print-runtime-worker.php (سرویس داخلی چاپ تحت مالکیت SOKNA Local)',
