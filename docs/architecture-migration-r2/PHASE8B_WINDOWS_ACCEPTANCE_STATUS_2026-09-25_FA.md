...
- Run `36198723040` روی `da6a8b09d409c3421a952ae43554c43635614c4f`: relocation Apache تأیید شد و preflight واقعی `new` مسیر managed config را درست یافت. Business setup در seed منو با `Call to undefined function normalize_sellable_kind()` متوقف شد؛ `default_menu_seed.php` هنگام Setup مستقیم dependency خود یعنی `sellable.php` را load نمی‌کرد. اصلاح جاری dependency را صریح می‌کند. preview Inno نصب/Runtime/Print Worker را با موفقیت انجام داد و سپس خود fixture به‌اشتباه دنبال CAای گشت که در مسیر فنی `SkipHttps` نباید ساخته شود؛ assertion با scope واقعی preview هم‌راستا شد.
