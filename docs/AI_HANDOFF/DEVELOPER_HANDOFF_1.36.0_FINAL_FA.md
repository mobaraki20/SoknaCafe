# هنداور توسعه Sokna 1.36.0

## Promotion نهایی از RC4

نسخه `1.36.0` از آخرین نسخه منتشرشده `1.36.0-rc.4` Promotion شده است. تغییر این مرحله فقط هویت نسخه، Cache، مستندات انتشار و بسته‌بندی است؛ Business Logic، Database Schema و Workflowهای عملیاتی نسبت به RC4 تغییر نکرده‌اند.
...
- نسخه و Cache Service Worker روی `1.36.0` هم‌هویت‌اند. مجوز صریح Owner برای انتشار نهایی در ۲۴ اوت ۲۰۲۶ دریافت شد.

## 1. وضعیت پروژه

Sokna یک Modular Monolith با PHP 8.1+ و MySQL/MariaDB است. این نسخه نهایی خط 1.36 و همچنان Pre-Operational است. مقیاس مرجع ۱۰۰–۱۱۰ مهمان هم‌زمان و تیم عملیاتی ۸–۹ نفر است. Promotion نهایی Behavior-preserving است.
...
- Release/tests/docs: `VERSION.txt`, `service-worker.js`, `CHANGELOG_FA.md`, `RELEASE_NOTES_1.36.0_FINAL_FA.md`, `tests/v1360-rc4-ui-handoff-contract.py`, گزارش و Handoff نهایی.

`release-manifest.json` بسته Update نهایی شامل ۳۸ فایل تغییرکرده و صفر Delete نسبت به Full Project منتشرشده RC4 است. بسته کامل ۵۶۶ فایل دارد و از همین Source نهایی ساخته شده است. Hash قطعی در فایل SHA256 انتشار ثبت شده است.

## 5. آزمون و محدودیت محیط

نتیجه کامل در `docs/TEST_REPORT_V1.36.0_FINAL_FA.md` ثبت شده است. قرارداد ZIP، Hash/Size مانیفست و هم‌ارزی ایستای بسته نهایی بررسی می‌شوند. در محیط تحویل PHP/DB/Chromium وجود نداشت؛ بنابراین هیچ ادعای PHP lint، اجرای واقعی Updater، DB PASS، Browser PASS، Print PASS یا Production deployment نشده است.

## 6. دستور کار ایجنت بعدی

1. روی محیط PHP 8.1+ دستور `tests/run-1360-dev-gate.sh` را اجرا کند.
2. Full Project منتشرشده RC4 را Clone کند و بسته `1.36.0-rc.4 → 1.36.0` را با Updater واقعی اعمال کند.
3. `RC4 + Final Update == 1.36.0 Source` را با Missing/Extra/HashDiff صفر ثابت کند.
...
7. فقط پس از صفرشدن Blockerها، Artifact نهایی را روی محیط Production استقرار دهد.

## 7. Rollback و ایمنی

- نسخه نهایی Migration ندارد؛ Rollback فایل‌ها از Restore Point Updater RC4 انجام می‌شود.
